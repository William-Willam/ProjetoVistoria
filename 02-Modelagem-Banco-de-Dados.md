# Modelagem de Banco de Dados — AutoVistor

Banco: MySQL 8, engine InnoDB, charset utf8mb4. Migrações versionadas via Flyway (`V1__init.sql`, `V2__...`).

## Diagrama de entidades (visão lógica)

```mermaid
erDiagram
    CLIENTE ||--o{ VEICULO : possui
    CLIENTE ||--o{ AGENDAMENTO : solicita
    VEICULO ||--o{ AGENDAMENTO : "é vistoriado em"
    FUNCIONARIO ||--o{ AGENDAMENTO : "é designado a"
    AGENDAMENTO ||--|| VISTORIA : origina
    FUNCIONARIO ||--o{ VISTORIA : realiza
    VISTORIA ||--|| LAUDO : gera
    VISTORIA ||--|| PAGAMENTO : cobra
    PAGAMENTO ||--o| BOLETO : "pode gerar"
    PAGAMENTO ||--o| NOTA_FISCAL : "confirma e gera"
    PAGAMENTO ||--o| LANCAMENTO_CAIXA : "confirma e gera"
    FUNCIONARIO ||--o{ DESLIGAMENTO : "pode ter"
```
...
CREATE TABLE agendamento (
    ...
    -- corrige a lacuna do DF-Vistoria: impede 2 agendamentos ativos no mesmo horário para o mesmo vistoriador
...
### boleto  *(novo — não existia no DF-Vistoria)*
```sql
CREATE TABLE boleto (
...
### desligamento_funcionario  *(FK corrigida)*
```sql
CREATE TABLE desligamento_funcionario (
    id                BIGINT PRIMARY KEY AUTO_INCREMENT,
    id_funcionario    BIGINT NULL,          -- nullable de propósito
    nome_funcionario  VARCHAR(150) NOT NULL, -- snapshot: sobrevive mesmo se o funcionário for removido
...
```
> No DF-Vistoria, essa FK usava `ON DELETE CASCADE`, o que apagava o próprio registro de histórico ao excluir o funcionário. Aqui, o registro é um **snapshot** (nome e matrícula copiados no momento do desligamento) e a FK usa `SET NULL`, então o histórico sobrevive mesmo após a exclusão do funcionário.

## Observações de design
- Todas as PKs/FKs usam `BIGINT` em vez de `INT` — mais folga para crescimento.
- Enums em maiúsculo (`APROVADO`, `PENDENTE`) para bater 1:1 com os enums Java do backend, sem necessidade de conversão manual (diferente do que ocorria no DF-Vistoria, onde havia mapeamento manual entre enum Java e string do banco).
- `agendamento.id_funcionario` é nullable porque a designação do vistoriador pode acontecer depois da criação do agendamento (RF005).
